...
{
    public class NhlClient(string apiKey) : Interfaces.ISportClient
    {
        private readonly NHLv3ScoresClient _scoresClient = new(apiKey);

        public string SportKey => "icehockey_nhl";
        ...
        public async Task<List<Interfaces.StadiumInfo>> GetStadiumsAsync()
        {
            ...
            return [.. stadiums.Select(s => new Interfaces.StadiumInfo
            {
                ...
                Country = s.Country,
                Capacity = s.Capacity,
                IsDome = true // NHL arenas are indoor
            })];
        }
